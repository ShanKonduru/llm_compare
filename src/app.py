    ...
    def get_ollama_gpu_mem_usage() -> float:
        ...
        try:
            result = subprocess.run(
                ["ollama", "ps"], capture_output=True, text=True, check=True, timeout=5
            )
            ...
            st.error(
                "The 'ollama' command was not found. Please ensure Ollama is installed and in your system's PATH."
            )
        except subprocess.TimeoutExpired:
            st.error("The 'ollama ps' command timed out.")
        except Exception as e:
            st.error(
                f"An unexpected error occurred while parsing 'ollama ps' output: {e}"
            )
    ...
    def _get_local_llms(_self) -> list[str]:
        ...
        try:
            ...
            return [
                model["model"]
                for model in models
                if model["details"]["family"] != "text-embedding"
                and "embed" not in model["model"]
            ]
        except Exception as e:
    ...
    def run_benchmark(self, model_names: list[str], prompts: list[str]) -> list[dict]:
        ...
        for model_name in model_names:
            for prompt in prompts:
                status_text.text(
                    f"Querying {model_name} with prompt: '{prompt[:30]}...'"
                )
                ...
                progress_bar.progress(
                    percent_complete, text=f"Benchmarking... {percent_complete}%"
                )
...
def main():
    ...
    if not local_llms:
        st.warning(
            "No local Ollama models found. Please ensure Ollama is running and you have pulled at least one model (e.g., 'ollama pull llama2')."
        )
        st.stop()
